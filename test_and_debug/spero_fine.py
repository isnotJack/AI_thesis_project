...
immagini = documenti_a_immagini(docs, tetto_totale=24)
ia.immagini = immagini
prompt, imgs = costruisci_prompt(ia)
print(f"[num_ctx=65536, tetto=24] {len(imgs)} immagini, invio...", flush=True)
t0 = time.time()
ollama_client.estrai(prompt, imgs, schema)
print(f"[num_ctx=65536, UKR 2024-Q3, 24 immagini] {time.time()-t0:.1f}s")
