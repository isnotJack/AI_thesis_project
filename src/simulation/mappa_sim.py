...
/* zoom/pan */
let vb={x:0,y:0,w:1000,h:500},HOME=1000;
...
function fit(){const P=NODI.map(n=>proj(n.lon,n.lat));if(!P.length){setVB();return;}
  ...
  if(w/h>A)h=w/A;else w=h*A;vb={x:cx-w*(PL/Wc+.5),y:cy-h/2,w:w*(W/Wc),h};HOME=vb.w;setVB();}
...
svg.addEventListener('wheel',e=>{e.preventDefault();const[mx,my]=toSvg(e.clientX,e.clientY);
  const f=e.deltaY<0?.82:1/.82,nw=Math.max(60,Math.min(1400,vb.w*f)),nh=vb.h*(nw/vb.w);
  vb.x=mx-(mx-vb.x)*(nw/vb.w);vb.y=my-(my-vb.y)*(nh/vb.h);vb.w=nw;vb.h=nh;setVB();},{passive:false});
